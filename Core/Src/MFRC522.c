...
// Version 2.0 (0x92)
// NXP Semiconductors; Rev. 3.8 - 17 September 2014; 16.1.1 self-test
const byte MFRC522_firmware_referenceV2_0_CLONE[]  = {
		0x00, 0x2E, 0xC6, 0xB5, 0x56, 0xA5, 0xF1, 0x71,
		0x91, 0xFD, 0xFD, 0xB1, 0xDE, 0x22, 0x7D, 0x07,
		0x7D, 0x48, 0xA5, 0x75, 0xBC, 0x39, 0x59, 0x53,
		0x6B, 0x69, 0xAF, 0xDA, 0xC3, 0xB8, 0x86, 0x3C,
		0xA0, 0xEF, 0x1E, 0xD3, 0xCC, 0xE7, 0xB2, 0xA4,
		0xEA, 0xFC, 0x51, 0x3E, 0x77, 0x9A, 0x71, 0x6B,
		0x92, 0x9D, 0x88, 0x6E, 0x82, 0xC9, 0x7A, 0xCF,
		0x3A, 0xEC, 0xD7, 0xD2, 0xC4, 0xF6, 0xDC, 0x0C,
};
// Clone
...
boolean PCD_PerformSelfTest() {
	...
	switch (version) {
		...
		case 0x92:	// Version 2.0
			...
			referenceCLONE = MFRC522_firmware_referenceV2_0_CLONE;
			break;
...
boolean MIFARE_OpenUidBackdoor(boolean logErrors) {
	...
	if(status != STATUS_OK) {
		if(logErrors) {
			Serial.println("Card did not respond to 0x40 after HALT command. Are you sure it is a UID changeable one?",STR);
			Serial.print("Error name: ",STR);
			Serial.println(GetStatusCodeName(status),STR);
		}
		return false;
	}
	if (received != 1 || response[0] != 0x0A) {
		if (logErrors) {
			Serial.print("Got bad response on backdoor 0x40 command: ",STR);
			Serial.print(F(&response[0]), HEX);
			Serial.print(" (",STR);
			Serial.print(F(&validBits),DEC);
	...
	if(status != STATUS_OK) {
		if(logErrors) {
			Serial.println("Error in communication at command 0x43, after successfully executing 0x40",STR);
			Serial.print("Error name: ",STR);
			Serial.println(GetStatusCodeName(status),STR);
		}
		return false;
	...
	if (received != 1 || response[0] != 0x0A) {
		if (logErrors) {
			Serial.print("Got bad response on backdoor 0x43 command: ",STR);
			Serial.print(F(&response[0]), HEX);
			Serial.print(" (",STR);
			Serial.print(F(&validBits),DEC);
			Serial.print(" valid bits)\r\n",STR);
